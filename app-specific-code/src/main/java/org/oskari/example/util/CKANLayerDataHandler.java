...
public class CKANLayerDataHandler {
  ...
  private static String gsUrl = PropertyUtil.get(
    "geoserver.url",
   ...
   */
  public static void addLayersFromCKANJSONResource(
    JSONObject resource,
    boolean isPrivateResource,
    Connection connection,
    CKANOrganization organization
  )
    throws ServiceException {
    CapabilitiesCacheService capabilitiesService = OskariComponentManager.getComponentOfType(
      CapabilitiesCacheService.class
    );
    ...
    if (updateNeeded) {
      ...
      String format = (resource.get("format") != null)
        ? (String) resource.get("format")
        : "No format defined!";
      String user = (resource.get("username") != null)
        ? (String) resource.get("username")
        : "";
      String pw = (resource.get("password") != null)
      ...
      switch ((format).toLowerCase()) {
        ...
        default:
          LOG.info(
            String.format("No match for resource data format (%s).", format)
          );
      }
    } else {
      LOG.info(
        String.format(
          "Resource %s is up-to-date, skipping...",
          resource.get("name")
        )
      );
    }
  ...
  private static boolean checkResourceStatus(
    JSONObject resource,
    Connection connection
  ) {
    boolean updateNeeded = false;
    String uuid = (String) resource.get("id");
    String lastModified = (resource.get("last_modified") != null)
      ? (String) resource.get("last_modified")
      : (String) resource.get("created");
    updateNeeded = resourceNeedsUpdate(connection, uuid, lastModified);

    return updateNeeded;
  }

  private static boolean resourceNeedsUpdate(
    Connection connection,
    String uuid,
    String lastModified
  ) {
    ...
    String sql =
      "SELECT * FROM oskari_ckan_dataset_resource_log WHERE resource_uuid = '" +
      uuid +
      "';";
    ...
    if (
      ...
      (
        currentModifiedDate.isEqual(lastModifiedDate) ||
        currentModifiedDate.isAfter(lastModifiedDate)
      )
    ) {
      return false;
    }
  ...
  private static void addOrUpdateResourceInfo(
    JSONObject resource,
    Connection connection
  ) {
    String uuid = (String) resource.get("id");
    ...
    String sql =
      "INSERT INTO \"oskari_ckan_dataset_resource_log\" (last_modified, resource_uuid)\n" +
      "VALUES\n" +
      "('" +
      lastModified +
      "', '" +
      uuid +
      "')\n" +
      "ON CONFLICT (resource_uuid) DO UPDATE SET last_modified = '" +
      lastModified +
      "';";
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
      ...
      LOG.error(
        "Error while updating Oskari CKAN resource log." + ex.toString()
      );
    }
  }

  private static void addWMSLayers(
    JSONObject resource,
    Connection connection,
    ...
    if (
      (resource.get("version") != null) &&
      !((String) resource.get("version")).isEmpty()
    ) {
      version = (String) resource.get("version");
    }

    String mainGroupName = (resource.get("name") != null)
      ? (String) resource.get("name")
      : "Misc Layers";
    LOG.debug(
      String.format(
        "Getting WMS capabilities from %s (version %s)",
        url,
        version
      )
    );
    org.json.JSONObject json = GetGtWMSCapabilities.getWMSCapabilities(
      capabilitiesService,
      url,
      user,
      pw,
  ...
  private static void addWMTSLayers(
    JSONObject resource,
    Connection connection,
    CapabilitiesCacheService capabilitiesService,
    String url,
    ...
    LOG.debug(
      String.format(
        "Getting WMTS capabilities from %s (version %s)",
        url,
        version
      )
    );
    OskariLayerCapabilities caps = capabilitiesService.getCapabilities(
      url,
    ...
    try {
      ...
      org.json.JSONObject resultJSON = WMTSCapabilitiesParser.asJSON(
        wmtsCaps,
        url,
        currentCrs
      );
  ...
  private static void addWFSLayers(
    JSONObject resource,
    Connection connection,
    String url,
    String user,
    ...
    LOG.debug(
      String.format(
        "Getting WFS capabilities from %s (version %s)",
        url,
        version
      )
    );
    org.json.JSONObject json = GetGtWFSCapabilities.getWFSCapabilities(
      url,
      user,
      pw,
      version,
  ...
  private static void addLayers(
    ...
    try {
      ...
      String forcedSRS = PropertyUtil.get(
        "ckan.integration.ckanapi.layer.forcedsrs",
        "EPSG:3857"
      );
      ...
      org.json.JSONObject locale = LayerJSONHelper.getIDPLocale(
        mainGroupName,
        mainGroupName
      );
      if (useDefaultLocales) {
        locale =
          LayerJSONHelper.getDefaultLocale(
            mainGroupName,
            mainGroupName,
            mainGroupName
          );
      ...
      int groupId = LayerHelper.addMainGroup(
        mainGroupName,
        mainGroupName,
        mainGroupName
      );
      ...
      org.json.JSONObject layerPermissions = LayerJSONHelper.getRolePermissionsJSONForRoleAndAdmin(
        organization.getName()
      );
      // If resource is marked private in CKAN, only allow admins to see it!
      if (isPrivateResource) {
        layerPermissions =
          LayerJSONHelper.getRolePermissionsJSONForPrivateResource(
            organization.getName()
          );
      }
      ...
      for (int i = 0; i < layers.length(); i++) {
        ...
        org.json.JSONObject layerLocale = LayerJSONHelper.getIDPLocale(
          layerTitle,
          layerTitle
        );
        if (useDefaultLocales) {
      ...
      int addedCount = LayerHelper.addLayers(
        layersToAdd,
        LayerHelper.getLayerGroups(groupId),
        true,
        connection
      );
      LOG.debug(
        String.format(
          "Added %d layer(s) from %s to group %s.",
          addedCount,
          url,
          mainGroupName
        )
  ...
  private static void addShpFileAsLayer(
    JSONObject resource,
    Connection connection,
    CapabilitiesCacheService capabilitiesService,
    String url,
    ...
    throws ServiceException {
    boolean publishWFS = (
        resource.get("publish_wfs") != null &&
        !((String) resource.get("publish_wfs")).equals("")
      )
      ? Boolean.valueOf((String) resource.get("publish_wfs"))
      : true;
    boolean removeSpacesFromShpName = PropertyUtil.getOptional(
      "ckan.integration.ckanapi.shp.removespaces",
      false
    );
    boolean createResourceWorkspaces = PropertyUtil.getOptional(
      "ckan.integration.ckanapi.shp.resourceworkspaces",
      true
    );
    boolean addForceProxy = PropertyUtil.getOptional(
      "ckan.integration.ckanapi.shp.forceproxy",
      false
    );
    String sysAdminAPIKey = PropertyUtil.get(
      "ckan.integration.ckanapi.sysadmin.apikey",
      null
    );

    String workspaceName = organization
      .getName()
      .replaceAll("[^a-zA-Z0-9]+", "_");
    String storeName = "shp_store";
    if (resource.get("name") != null) {
      storeName =
        ((String) resource.get("name")).replaceAll("[^a-zA-Z0-9]+", "_");
    }
    ...
    try {
      ...
      uploadSldToGeoServer(
        gsUrl,
        new File(dataFilePath),
        workspaceName,
        FileHelper.getFileNameFromZip(dataFilePath, "shp")
      );

      String wmsUrl = String.format("%s/%s/wms", gsUrl, workspaceName);
      resource.put(
        "name",
        String.format("%s (local data)", organization.getTitle())
      );
      if (publishWFS) {
  ...
  private static void uploadShpFileToGeoServer(
    ...
    httpPut.setEntity(
      new FileEntity(shpFileZip, ContentType.create("application/zip"))
    );
    httpPut.setHeader("Authorization", authHeaderValue);

    LOG.info(
      String.format(
        "Uploading shp to GeoServer (request: %s) ",
    ...
    LOG.info(
      String.format("Got response from GeoServer: %s", responseBody.toString())
    );
  }

  private static void uploadSldToGeoServer(
    String gsUrl,
    File styleFile,
    String workspaceName,
    String shpName
    ...
    httpPost.setEntity(
      new FileEntity(styleFile, ContentType.create("application/zip"))
    );
    ...
    LOG.info(
      String.format(
        "Uploading style to GeoServer (request: %s) ",
    ...
    if (styleName != null && shpName != null) {
      HttpPut httpPut = new HttpPut(
        String.format("%s/rest/layers/%s:%s", gsUrl, workspaceName, shpName)
      );
      httpPut.setEntity(
        new StringEntity(
          String.format(
            "<layer><defaultStyle><name>%s</name></defaultStyle></layer>",
            styleName
          ),
  ...
  private static void addGeoTIFFAsLayer(
    JSONObject resource,
    Connection connection,
    CapabilitiesCacheService capabilitiesService,
    String url,
    ...
    throws ServiceException {
    boolean createResourceWorkspaces = PropertyUtil.getOptional(
      "ckan.integration.ckanapi.geotiff.resourceworkspaces",
      false
    );
    boolean addForceProxy = PropertyUtil.getOptional(
      "ckan.integration.ckanapi.geotiff.forceproxy",
      false
    );
    String sysAdminAPIKey = PropertyUtil.get(
      "ckan.integration.ckanapi.sysadmin.apikey",
  ...
  private static void uploadGeoTIFFToGeoServer(
    String workspaceName,
    String storeName,
    ResponseHandler<String> responseHandler,
    String gsUrl,
    ...
    HttpPut httpPut = new HttpPut(
      String.format(
        "%s/rest/workspaces/%s/coveragestores/%s/file.geotiff",
        gsUrl,
        workspaceName,
        storeName
      )
    ...
    httpPut.setEntity(
      new FileEntity(tiffFile, ContentType.create("image/tiff"))
    );
    httpPut.setHeader("Authorization", authHeaderValue);

    LOG.info(
      String.format(
        "Uploading GeoTIFF to GeoServer (request: %s) ",
        httpPut.getRequestLine()
      )
    );
  ...
  private static void createGeoServerWorkspace(
    String workspaceName,
    ResponseHandler<String> responseHandler,
    String gsUrl
  )
    ...
    HttpPost httpPost = new HttpPost(
      String.format("%s/rest/workspaces", gsUrl)
    );
    httpPost.setEntity(
      new StringEntity(
        String.format("<workspace><name>%s</name></workspace>", workspaceName),
        ContentType.create("text/xml")
      )
    );
    httpPost.setHeader("Authorization", authHeaderValue);
    ...
    LOG.info(
      String.format(
        "Creating GeoServer workspace (request: %s) ",
        httpPost.getRequestLine()
      )
    );
  ...
  private static void deleteGeoServerWorkspace(
    String workspaceName,
    ResponseHandler<String> responseHandler,
    String gsUrl
  )
    ...
    HttpDelete httpDelete = new HttpDelete(
      String.format("%s/rest/workspaces/%s?recurse=true", gsUrl, workspaceName)
    );
    httpDelete.setHeader("Authorization", authHeaderValue);

    LOG.info(
      String.format(
        "Deleting GeoServer workspace (request: %s) ",
  ...
  private static void deleteGeoServerStore(
    String storeType,
    String workspaceName,
    String storeName,
    ResponseHandler<String> responseHandler,
    ...
    HttpDelete httpDelete = new HttpDelete(
      String.format(
        "%s/rest/workspaces/%s/%ss/%s?recurse=true",
        gsUrl,
        workspaceName,
        storeType,
        storeName
    ...
    LOG.info(
      String.format(
        "Deleting GeoServer %s (request: %s) ",
        storeType,
        httpDelete.getRequestLine()
      )
    );
