...
public class STLayerSave extends AbstractLayerAdminHandler {
  ...
  private static final String PARAM_DOWNLOAD_PERMISSIONS =
    "downloadPermissions";
  private static final String PARAM_EMBEDDED_PERMISSIONS =
    "embeddedPermissions";
  ...
  private static final String PARAM_CAPABILITIES_UPDATE_RATE_SEC =
  ...
  private static final String PARAM_GML_GEOMETRY_PROPERTY =
    "GMLGeometryProperty";
  ...
  private static final String PARAM_FEATURE_NAMESCAPE_URI =
    "featureNamespaceURI";
  private static final String PARAM_FEATURE_PARAMS_LOCALES =
    "featureParamsLocales";
  private static final String PARAM_FEATURE_TYPE = "featureType";
  private static final String PARAM_GEOMETRY_NAMESPACE_URI =
  ...
  private static final String PARAM_SELECTED_FEATURE_PARAMS =
  ...
  private static final String ERROR_UPDATE_OR_INSERT_FAILED =
    "update_or_insert_failed";
  private static final String ERROR_NO_LAYER_WITH_ID = "no_layer_with_id:";
  private static final String ERROR_OPERATION_NOT_PERMITTED =
    "operation_not_permitted_for_layer_id:";
  private static final String ERROR_MANDATORY_FIELD_MISSING =
    "mandatory_field_missing:";
  private static final String ERROR_INVALID_FIELD_VALUE =
    "invalid_field_value:";
  private static final String ERROR_FE_PARSER_CONFIG_MISSING =
    "FE WFS feature parser config missing";
  ...
  public void handleGet(ActionParameters params) throws ActionException {
    ...
    ResponseHelper.writeResponse(
      params,
      "try a post req next... " + params.getRequest().getParameterMap()
    );
  }
  ...
  public void handlePost(ActionParameters params) throws ActionException {
    ...
    if (ml == null) {
      throw new ActionParamsException(
        "Couldn't get the saved layer from DB - id:" + layerId
      );
    }

    // construct response as layer json
    final JSONObject layerJSON = OskariLayerWorker.getMapLayerJSON(
      ml,
      params.getUser(),
      params.getLocale().getLanguage(),
      params.getHttpParam(PARAM_SRS)
    );
    ...
    List<OskariLayerGroupLink> groupLinks = layerGroupLinkService.findByLayerId(
      layerId
    );
  ...
  private SaveResult saveLayer(final ActionParameters params)
      ...
      if (layer_id != -1) {
        ...
        if (!userHasEditPermission(params.getUser(), ml)) {
          throw new ActionDeniedException(
            ERROR_OPERATION_NOT_PERMITTED + layer_id
          );
        }
        ...
        if (maplayerGroups != null) {
          ...
          List<OskariLayerGroupLink> links = getMaplayerGroupLinks(
            ml.getId(),
            groupIds
          );
  ...
  private static int[] getMaplayerGroupIds(String maplayerGroups) {
    return Arrays
      .stream(maplayerGroups.split(","))
      .mapToInt(gid -> ConversionHelper.getInt(gid, -1))
      .filter(gid -> gid >= 0)
      .toArray();
  }

  private List<OskariLayerGroupLink> getMaplayerGroupLinks(
    final int layerId,
    final int[] groupIds
  ) {
  ...
  private boolean handleRequestToMapLayer(
    ...
    // organization id
    final DataProvider dataProvider = dataProviderService.find(
      params.getHttpParam(PARAM_GROUP_ID, -1)
    );
    ...
    ml.setBaseMap(
      ConversionHelper.getBoolean(params.getHttpParam(PARAM_IS_BASE), false)
    );
    ...
    ml.setName(
      params.getRequiredParam(
        PARAM_LAYER_NAME,
        ERROR_MANDATORY_FIELD_MISSING + PARAM_LAYER_NAME
      )
    );
    final String url = params.getRequiredParam(
      PARAM_LAYER_URL,
      ERROR_MANDATORY_FIELD_MISSING + PARAM_LAYER_URL
    );
    ml.setUrl(url);
    ...
    ml.setMinScale(
      ConversionHelper.getDouble(
        params.getHttpParam(PARAM_MIN_SCALE),
        ml.getMinScale()
      )
    );
    ml.setMaxScale(
      ConversionHelper.getDouble(
        params.getHttpParam(PARAM_MAX_SCALE),
        ml.getMaxScale()
      )
    );

    ml.setLegendImage(
      params.getHttpParam(PARAM_LEGEND_IMAGE, ml.getLegendImage())
    );
    ml.setMetadataId(
      params.getHttpParam(PARAM_METADATA_ID, ml.getMetadataId())
    );

    final String gfiContent = request.getParameter(PARAM_GFI_CONTENT);
    if (gfiContent != null) {
      // Clean GFI content
      final String[] tags = PropertyUtil.getCommaSeparatedList(
        "gficontent.whitelist"
      );
      HashMap<String, String[]> attributes = new HashMap<String, String[]>();
      HashMap<String[], String[]> protocols = new HashMap<String[], String[]>();
      String[] allAttributes = PropertyUtil.getCommaSeparatedList(
        "gficontent.whitelist.attr"
      );
      if (allAttributes.length > 0) {
        attributes.put(":all", allAttributes);
      }
      List<String> attrProps = PropertyUtil.getPropertyNamesStartingWith(
        "gficontent.whitelist.attr."
      );
      for (String attrProp : attrProps) {
        ...
        if (parts[parts.length - 2].equals("protocol")) {
          protocols.put(
            new String[] { parts[parts.length - 3], parts[parts.length - 1] },
      ...
      ml.setGfiContent(
        RequestHelper.cleanHTMLString(gfiContent, tags, attributes, protocols)
      );
    }
    ...
    ml.setCapabilitiesUpdateRateSec(
      params.getHttpParam(PARAM_CAPABILITIES_UPDATE_RATE_SEC, 0)
    );
    ...
    ml.setRealtime(
      ConversionHelper.getBoolean(
        params.getHttpParam(PARAM_REALTIME),
        ml.getRealtime()
      )
    );
    ml.setRefreshRate(
      ConversionHelper.getInt(
        params.getHttpParam(PARAM_REFRESH_RATE),
        ml.getRefreshRate()
      )
    );
  ...
  private void validateInsertLayer(
    final ActionParameters params,
    OskariLayer ml
  )
    throws ActionException {
    ...
    if (
      params.getHttpParam(PARAM_JOB_TYPE) != null &&
  ...
  private boolean handleWMSSpecific(
    final ActionParameters params,
    OskariLayer ml,
    ...
    try {
      ...
      WebMapService wms = OskariLayerCapabilitiesHelper.parseWMSCapabilities(
        data,
        ml
      );
      OskariLayerCapabilitiesHelper.setPropertiesFromCapabilitiesWMS(
        wms,
        ml,
        systemCRSs
      );
      ...
      ServiceException
      | WebMapServiceParseException
      | LayerNotFoundInCapabilitiesException ex
    ) {
  ...
  private boolean handleWMTSSpecific(
    final ActionParameters params,
    OskariLayer ml,
    Set<String> systemCRSs
  ) {
    try {
      ...
      OskariLayerCapabilitiesHelper.setPropertiesFromCapabilitiesWMTS(
        caps,
        ml,
        systemCRSs
      );
  ...
  private void handleWFSSpecific(
    final ActionParameters params,
    OskariLayer ml,
    Set<String> systemCRSs
  )
    ...
    if (
      ConversionHelper.getOnOffBoolean(
        params.getHttpParam(PARAM_MANUAL_REFRESH, "off"),
        false
      )
    ) {
    ...
    if (
      ConversionHelper.getOnOffBoolean(
        params.getHttpParam(PARAM_RESOLVE_DEPTH, "off"),
        false
      )
    ) {
    ...
    try {
      if (WFS3_0_0_VERSION.equals(ml.getVersion())) {
        WFS3Service service = WFS3Service.fromURL(
          ml.getUrl(),
          ml.getUsername(),
          ml.getPassword()
        );
        OskariLayerCapabilitiesHelper.setPropertiesFromCapabilitiesWFS(
          service,
          ml,
          systemCRSs
        );
        ...
        ml.setCapabilities(
          GetGtWFSCapabilities.getLayerCapabilities(ml, systemCRSs)
        );
      }
    } catch (Exception e) {
      LOG.warn(
        "Couldn't update capabilities for WFS (" + ml.getVersion() + ") layer:",
        ml.getName(),
        e.getMessage()
      );
    }
  ...
  private String validateUrl(final String url) throws ActionParamsException {
    try {
      ...
      throw new ActionParamsException(
        ERROR_INVALID_FIELD_VALUE + PARAM_LAYER_URL + e.getMessage()
      );
    }
    return url;
  }

  private void addPermissionsForRoles(
    final OskariLayer ml,
